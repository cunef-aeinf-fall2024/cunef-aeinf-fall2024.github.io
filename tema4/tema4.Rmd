...
* **Generalizar** hallazgos de una muestra a una población determinada (inferencia)
...
* Generalizarse a la población de la que proceden los datos
...
sean generalizables a una población determinada, debemos muestrear los sujetos **al azar** de la población.
...
Como parte de la encuesta, se preguntaba a las mujeres una vez al año qué habían comido durante los tres días anteriores...
...
Existen tres posibles explicaciones de esta asociación entre comer cereales y tener un índice de masa corporal más bajo:
...
* En esta parte del curso, introduciremos herramientas básicas de modelización, poniendo
...
* Los modelos suelen ser paramétricos: $\mathcal {P}=\{F_{\theta }:\theta \in \Theta \}$
...
* Empecemos con un ejemplo sencillo: la variable respuesta es continua y hay una sola variable predictora.
...
El modelo de regresión lineal simple hace, implíctamente, 4 asunciones

1. **Linealidad**: el valor esperado de la variable respuesta corresponde con una función lineal de la covariable.
...
En este curso, no vamos a entrar en el estudio de estas asunciones, pero tened en cuenta que existen.
...
* Un modelo de regresión lineal simple parece razonable $y = \beta_0 + \beta_1 x + \epsilon$ con $\epsilon \sim \mathcal{N}(0, \sigma^2)$.

* Vamos a asumir que $\sigma^2$ es conocido luego los parámetros de interés son $\beta_0$ y $\beta_1$.
...
1. Demostrar que los valores que minimizan la expresión anterior son los estimadores de maxima verosimilitud de $\beta_0$ y $\beta_1$.

2. Calcular expresiones para los estimadores de $\beta_0$ y $\beta_1$.
...
* En este curso no nos centraremos en este aspecto. Únicamente trabajaremos con los estimadores puntuales y nos centraremos en cómo usarlos para explorar los datos.
...
* $y = \beta_0 + \beta_1 x + \epsilon$ no tendría sentido, $x$ no es un número!

* Podemos hacer $y = \beta_0 +\beta_1 \text{gen}\_\text{masc} + \epsilon$, donde $\text{gen}\_\text{masc}$ toma valor 1 para hombres y cero para mujeres. 
...
* Los efectos de diferentes covariables sobre el valor esperado de la variable dependiente son aditivos.
...
¿Cómo interpretas los coeficientes?
...
Si la muestra es .vocab[representativa] (escogida al azar de la población de interés), entonces podemos utilizar 
